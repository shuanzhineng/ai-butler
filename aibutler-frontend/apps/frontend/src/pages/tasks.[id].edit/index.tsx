...
const CreateTask = () => {
  ...
  const taskId = routeParams.taskId ? routeParams.taskId : 0;
  ...
  console.log(taskId)
  const isExistTask = taskId != 0 ? true : false
  ...
  useEffect(() => {
    basicFormInstance.setFieldsValue(taskData?.details);
  ...
  const handleSave = useCallback(
    async function (isFromCancel?: boolean) {
      ...
      return updateTaskConfig
        ...
        .then(() => {
          navigate(`/home/tasks/${taskData?.details.id}`);
    ...
    async function (isFromCancel) {
      ...
      if (isExistTask) {
        if (currentStep === StepEnum.Upload && !_.isEmpty(uploadFileList) && !attachmentsConnected.current) {
          await createSamples(
            ...
            _.chain(uploadFileList)
              ...
              .map((item) => ({
                attachement_ids: [item.details?.id!],
                data: {
                  fileNames: {
                    [item.details?.id!]: item.details?.local_path!,
                  },
                  result: {},
                  urls: {
                    [item.details?.id!]: item.details?.url!,
                  },
                },
              }))
              .value(),
        ...
        return addTask.mutateAsync(basicFormValues).then((res) => {
          console.log(res)
          const newTask = res?.details

          // 取消并保存时，跳转到任务列表页
          if (isFromCancel) {
            navigate('/home/tasks');
          } else if (newTask) {
            navigate(`/home/tasks/${newTask.id}/edit${location.search}#${StepEnum.Upload}`);
          }
          return Promise.reject();
        });
      }
        ...
        <TaskCreationContext.Provider value={taskCreationContextValue}>
          ...
          {previewVisible && (
            <PreviewFrame
              ...
              src={`/home/tasks/${taskData.id}/samples/${samples?.data?.[0].id}?noSave=true`}
            />
          )}
